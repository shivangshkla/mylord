...
(function(entries,appearOnScroll) {
    entries.forEach (entry => {
      if(!entry.isIntersecting){
          ...
          if (i == 0){
            entry.target.classList.add('fill-in-switzerland');
            appearOnScroll.unobserve(entry.target);
          }
          if (i == 1){
            entry.target.classList.add('fill-in-finland');
            appearOnScroll.unobserve(entry.target);
          }
          if (i == 2){
            entry.target.classList.add('fill-in-india');
            appearOnScroll.unobserve(entry.target);
          }
          if (i == 3){
            entry.target.classList.add('fill-in-denmark');
            appearOnScroll.unobserve(entry.target);
          }
          i = i+1;
      }
